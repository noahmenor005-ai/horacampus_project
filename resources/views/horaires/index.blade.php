...
<div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2 mb-3">
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        @can('create', \App\Models\Horaire::class)
            <a class="btn btn-primary" href="{{ route('horaires.create') }}"><i class="bi bi-plus-lg"></i> Programmer</a>
        @endcan
    </div>
</div>
    ...
    <div class="row g-2 align-items-end">
        ...
        <div class="col-md-3 col-lg-2">
            ...
            <select id="promotion_id" name="promotion_id" class="form-select">
                ...
                @foreach($promotions as $id => $label)<option value="{{ $id }}" @selected(request('promotion_id') == $id)>{{ $label }}</option>@endforeach
            </select>
        </div>
        <div class="col-md-3 col-lg-2">
            ...
            <select id="enseignant_id" name="enseignant_id" class="form-select">
                <option value="">Tous</option>
                @foreach($enseignants as $id => $label)<option value="{{ $id }}" @selected(request('enseignant_id') == $id)>{{ $label }}</option>@endforeach
            </select>
        </div>
        <div class="col-md-3 col-lg-2">
        ...
        <div class="col-md-3 col-lg-2">
            ...
            <select id="semestre_id" name="semestre_id" class="form-select">
                <option value="">Tous</option>
                @foreach($semestres as $id => $label)<option value="{{ $id }}" @selected(request('semestre_id') == $id)>{{ $label }}</option>@endforeach
            </select>
        </div>
        <div class="col-lg-2">
    ...
    <table class="table align-middle mb-0">
        ...
        @forelse($horaires as $h)
            <tr>
                ...
                <td class="text-end">
                    @can('update', $h)
                        <a class="btn btn-sm btn-outline-primary" href="{{ route('horaires.edit', $h) }}" title="Modifier"><i class="bi bi-pencil"></i></a>
                    @endcan
                    @can('delete', $h)
                        <form method="POST" action="{{ route('horaires.destroy', $h) }}" class="d-inline">
                            @csrf @method('DELETE')
